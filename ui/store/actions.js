...
export function getHexAddressFromLBChAddress(_address) {
  return async () => {
    try {
      const lbchAddress = await promisifiedBackground.getHexAddressFromLBCh(
        _address,
      );
      return lbchAddress;
    } catch (error) {
      console.log('[getHexAddressFromLBCh error]', error);
      return undefined;
    }
  };
}

export function getLBChAddressFromHexAddress(_address) {
  return async () => {
    try {
      const lbchAddress = await promisifiedBackground.getLBChAddressFromHex(
        _address,
      );
      return lbchAddress;
    } catch (error) {
      console.log('[getLBChAddressFromHexAddress error]', error);
      return undefined;
    }
  };
}
...
export function updateMetamaskState(newState) {
  ...
  return (dispatch, getState) => {
    ...
    if (
      nativeCurrency === 'LBCh' &&
      newState.lbchBalances[newSelectedAddress] !== undefined
    ) {
      console.log(
        '[account lbch balance action check]',
        newState.lbchBalances[newSelectedAddress].spendableBalance,
      );
      dispatch({
        type: actionConstants.UPDATE_LBCh_BALANCE,
        payload: {
          lbchBalances: {
            spendableBalance:
              newState.lbchBalances[newSelectedAddress].spendableBalance,
            pendingBalance:
              newState.lbchBalances[newSelectedAddress].pendingBalance,
          },
        },
      });
    }
...
export function setPrimaryAddressPreference(value) {
  return setPreference('isLBChAddressShow', value);
}
