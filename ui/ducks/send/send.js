...
/**
 * @typedef {Object} LBChBalances
 * @property {string} spendableBalance - spendable P2PK lbch balance
 * @property {string} pendingBalance - pending lbch balance
 * @property {string} [error] - error to display for lbch spendable balance fields.
 */
 ...
 * @property {LBChBalances} lbchBalances - The spendable lbch balance to use for max send amount.
 */
...
export const initializeSendState = createAsyncThunk(
  ...
  async ({ chainHasChanged = false } = {}, thunkApi) => {
    ...
    if (draftTransaction.asset.type === ASSET_TYPES.NATIVE) {
      lbchBalances = getLBChBalances(state);
      console.log('[check lbch balance from metamask duck]', lbchBalances);
    }
...
const slice = createSlice({
    ...
    updateAmountToMax: (state) => {
      ...
      if (draftTransaction.asset.type === ASSET_TYPES.TOKEN) {
        ...
        amount = subtractCurrencies(
          addHexPrefix(state.lbchBalances.spendableBalance),
        ...
        console.log(
          ...
          _gasTotal,
          amount,
          state.lbchBalances.spendableBalance,
        );
      }
      ...
      switch (true) {
        ...
        case draftTransaction.asset.type === ASSET_TYPES.NATIVE &&
          !isBalanceSufficient({
            ...
            balance: lbchBalances.spendableBalance,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(UPDATE_LBCh_BALANCE, (state, action) => {
        console.log('[update lbch balance]', action.payload);
        state.lbchBalances = { ...action.payload.lbchBalances };
      })
...
export function sendLBChAmountIsInError(state) {
  return Boolean(state[name].lbchBalances.error);
}
...
export function getSendErrors(state) {
  return {
    ...
    lbchBalances: state.send.lbchBalances.error,
  };
}
...
export function getLBChSpendableBalanceInString(state) {
  let balance = '0x0';
  const draftTransaction = getCurrentDraftTransaction(state);
  console.log(
    '[get lbch spendable balance]',
    draftTransaction,
    state.send.lbchBalances,
  );
  if (state.send.lbchBalances.spendableBalance) {
    const amount = subtractCurrencies(
      addHexPrefix(state.send.lbchBalances.spendableBalance),
