...
import {
  ...
  shortenAddress,
  stripHttpSchemes,
  getHexAddressFromLBCh,
  getLBChAddressFromHex
...
import { getCollectibles, getTokens, getCurrentProvider, isLBChAddressShow } from '../ducks/metamask/metamask';
import {
  TRANSACTION_TYPES,
...
export function useTransactionDisplayData(transactionGroup) {
  ...
  const isLBChAddressShowChk = useSelector(isLBChAddressShow);
  ...
  if (signatureTypes.includes(type)) {
    ...
    title = t('receive');
    prefix = '';
    subtitle = isLBChAddressShowChk ? t('fromAddress', [shortenAddress(getLBChAddressFromHex(senderAddress, chainId))]) : t('fromAddress', [shortenAddress(senderAddress)]);
  } else if (
    ...
    recipientAddress = getTokenAddressParam(tokenData);
    subtitle = isLBChAddressShowChk ? t('toAddress', [shortenAddress(getLBChAddressFromHex(recipientAddress, chainId))]) : t('toAddress', [shortenAddress(recipientAddress)]);
