...
export default class EnsInput extends Component {
  ...
  static propTypes = {
    ...
    lbchAddressBook: PropTypes.object,
    ...
    if (event.clipboardData.items?.length) {
      ...
      clipboardItem?.getAsString((text) => {
        ...
        if (this.isBase58(input) || !isHexString(input)) {
          hexAddress = getHexAddressFromLBCh(input);
        }
        if (
  ...
  convertAddress = (input) => {
    const { chainId, isLBChAddressShowCheck } = this.props;
    if (isLBChAddressShowCheck && isHexString(input) && input !== '') {
      const newAddress = getLBChAddressFromHex(input, chainId);
      return newAddress;
    } else if (
      ...
      const newAddress = getHexAddressFromLBCh(input);
      return newAddress;
    }
    return input;
  };
              ...
              <div className="ens-input__wrapper__input ens-input__wrapper__input--selected">
                <div className="ens-input__selected-input__title">
                  {this.convertAddress(selectedName) ||
                    (isLBChAddressShowCheck
                      ? lbchAddressBook[selectedAddress]
