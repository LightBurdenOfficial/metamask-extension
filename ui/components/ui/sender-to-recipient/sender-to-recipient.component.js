...
import { shortenAddress, getLBChAddressFromHex } from '../../../helpers/utils/util';
...
import { isLBChAddressShow, getCurrentProvider } from '../../../ducks/metamask/metamask';
import {
  DEFAULT_VARIANT,
  CARDS_VARIANT,
  FLAT_VARIANT,
...
function SenderAddress({
  ...
  if (!addressCopied) {
    tooltipHtml = addressOnly ? (
      ...
      <p>
        {shortenAddress(isLBChAddressShowCheck ? getLBChAddressFromHex(checksummedSenderAddress, chainId) : checksummedSenderAddress)}
        <br />
        {t('copyAddress')}
      </p>
    );
  ...
  return (
    <div
      ...
      onClick={() => {
        ...
        copyToClipboard(isLBChAddressShowCheck ? getLBChAddressFromHex(checksummedSenderAddress, chainId) : checksummedSenderAddress);
        if (onSenderClick) {
          onSenderClick();
        }
      }}
    >
      ...
      >
        <div className="sender-to-recipient__name">
          {addressOnly ? (
            <span>
              {`${senderName || shortenAddress(isLBChAddressShowCheck ? getLBChAddressFromHex(checksummedSenderAddress, chainId) : checksummedSenderAddress)}`}
            </span>
          ) : (
            senderName
          )}
...
SenderAddress.propTypes = {
  ...
  isLBChAddressShowCheck: PropTypes.bool,
      ...
      >
        ...
        <div className="sender-to-recipient__name">
          {addressOnly
            ? recipientNickname ||
              recipientEns ||
              shortenAddress(isLBChAddressShowCheck ? getLBChAddressFromHex(checksummedRecipientAddress, chainId) : checksummedRecipientAddress)
            : recipientNickname ||
              recipientEns ||
              recipientName ||
              t('newContract')}
...
function SenderToRecipient({
  ...
  const { isLBChAddressShowCheck, chainId } = props;
  return (
    <div className={classnames('sender-to-recipient', variantHash[variant])}>
      <SenderAddress
        ...
        isLBChAddressShowCheck={isLBChAddressShowCheck}
      />
      <Arrow />
      {recipientAddress ? (
