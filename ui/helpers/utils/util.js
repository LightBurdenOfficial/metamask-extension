...
export function getLBChAddressFromHex(_address, _chainId) {
  let version;
  switch (_chainId) {
    case '0x22B8':
      version = 58;
...
export function getHexAddressFromLBCh(_address) {
  const hexAddress = lbch.address.fromBase58Check(_address).hash.toString('hex')
  return `0x${hexAddress}`
}

export function getLPPTokenTrackerLink(_tokenTrackerLink) {
  const newLink = _tokenTrackerLink.replace(/token/g, 'lpp20');
  return newLink;
}
