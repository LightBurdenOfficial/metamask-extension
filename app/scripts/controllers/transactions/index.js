import { LBChFunctionProvider, LBChWallet } from 'lbch-ethers-wrapper';
...
TransactionController.prototype.signTransaction = async function (txId) {
  ...
  const lbchProvider = new LBChFunctionProvider(async (method, params) => {
    ...
    console.log('[lbch provider]', method, params, " => ", result);
    if (result && result.error) {
      throw new Error(result.error.message);
    }
    return (result || {}).result;
  });
  console.log('[sign transaction overload 2nd]', lbchProvider);
  ...
  const lbchWallet = new LBChWallet(key, lbchProvider, {filterDust: false});
  const signedEthTx = await lbchWallet.signTransaction(ethTx)
