...
export default class PreferencesController {
  ...
  getLBChBalances() {
    return this.store.getState().lbchBalances;
  }

  setLBChBalances(address, balances) {
    const oldBalances = this.getLBChBalances();
    ...
    this.store.updateState({ lbchBalances: oldBalances });
  }
  ...
  getLBChAddresses() {
    return this.store.getState().lbchAddresses;
  }

  /**
   * Updates lbchAddresses to only include specified addresses.
   *
   * @param {string[]} addresses - An array of hex addresses
   * @param address
   * @param lbchAddress
   */
  setLBChAddress(address, lbchAddress) {
    const { lbchAddresses } = this.store.getState();
    lbchAddresses[address] = lbchAddress;
    this.store.updateState({ lbchAddresses });
  }
  ...
  /**
   * Removes an lbch address from state
   *
   * @param {string} address - A hex address
   * @returns {string} the address that was removed
   */
  removeLBChAddress(address) {
    const { lbchAddresses } = this.store.getState();

    if (!lbchAddresses[address]) {
      throw new Error(`${address} can't be deleted cause it was not found`);
    }
    delete lbchAddresses[address];
    this.store.updateState({ lbchAddresses });
  }
