...
import { LBChWallet } from 'lbch-ethers-wrapper';
...
export default class MetamaskController extends EventEmitter {
  ...
  constructor(opts) {
        ...
        return new Promise((resolve, reject) => {
            ...
            ethQuery.sendAsync({
                ...
                if (err) {
                    ...
                    const hasBug = result === "LBCh ETHTestRPC/ethereum-js";
    ...
    this.networkController.on(NETWORK_EVENTS.NETWORK_DID_CHANGE, async () => {
      ...
      try {
        ...
        const lbchAccounts = await this.preferencesController.getLBChAddresses();
        Object.keys(lbchAccounts).forEach((item) => {
          this.setLBChAddressFromHexAddress(item);
        });
      } catch (error) {
  ...
  getApi() {
    ...
    return {
      ...
      // LBCh
      // set native currency to LBCh
      setNativeCurrency: nodeify(this.setNativeCurrency, this),
      // get Hex address from LBCh
      getHexAddressFromLBCh: nodeify(this.getHexAddressFromLBCh, this),
      // get lbch address from hex
      getLBChAddressFromHex: nodeify(this.getLBChAddressFromHex, this),
  ...
  async createNewVaultAndKeychain(password) {
    ...
    try {
      ...
      if (accounts.length > 0) {
        ...
        await this.setLBChBalances(accounts[0]);
        await this.setLBChAddressFromHexAddress(accounts[0]);
      } else {
        ...
        await this.setLBChBalances(addresses[0]);
        await this.setLBChAddressFromHexAddress(addresses[0]);
      }
  ...
  async exportAccount(address) {
    await this.MonekyPatchLBChExportAccount();
  ...
  async addNewAccount(accountCount) {
    ...
    if (Object.keys(oldIdentities).length === accountCount) {
      ...
      await this.setLBChBalances(newAccounts);
      await this.setLBChAddressFromHexAddress(newAccounts[0]);
   ...
   */
  setNativeCurrency() {
    this.monkeyPatchLBChSetCurrency();
  }
  ...
  /**
   * A method for getting hex address from lbch address.
   *
   * @param _lbchAddress
   */
  async getHexAddressFromLBCh(_lbchAddress) {
    return await this.getHexAddressFromLBChAddress(_lbchAddress);
  }

  /**
   * A method for getting lbch address from hex address.
   *
   * @param _lbchAddress
   */
  async getLBChAddressFromHex(_lbchAddress) {
    return await this.getLBChAddressFromHexAddress(_lbchAddress);
  }
}
...
[
  ...
  MetamaskController.prototype[methodToOverload] = function () {
    this.monkeyPatchLBChAddressGeneration();
    this.monkeyPatchLBChAddressImport();
    this.MonekyPatchLBChExportAccount();
...
MetamaskController.prototype.addNewKeyring = function (p, o) {
  // this.monkeyPatchLBChAddNewKeyring();
  this.monkeyPatchLBChAddressImport();
  this.MonekyPatchLBChExportAccount();
  return this._addNewKeyring.apply(this, arguments);
};

MetamaskController.prototype.monkeyPatchLBChAddressGeneration = function (
  password,
) {
  ...
  for (let i = 0; i < this.keyringController.keyringTypes.length; i++) {
    ...
    switch (type) {
      case 'HD Key Tree':
        console.log('monkey patching LBCh address generation into hd key tree');
        this.monkeyPatchHDKeyringAddNewKeyring();
        this.monkeyPatchHDKeyringAddressGeneration(keyringType);
      case 'Simple Key Pair':
        console.log(
          'monkey patching LBCh address generation into simple key pair',
        );
        this.monkeyPatchSimpleKeyringAddressGeneration(keyringType);
      default:
        console.log(
          `LBCh address generation support for ${type} is not yet supported`,
        );
    }
  }
};

MetamaskController.prototype.monkeyPatchLBChAddressImport = function () {
  for (let i = 0; i < this.keyringController.keyringTypes.length; i++) {
    const keyringType = this.keyringController.keyringTypes[i];
    if (keyringType.prototype.hasOwnProperty('_monkeyPatched')) {
      continue;
    ...
    switch (type) {
      case 'HD Key Tree':
        console.log('monkey patching LBCh address import into hd key tree');
        this.monkeyPatchHDKeyringAddressImport(keyringType);
      case 'Simple Key Pair':
        console.log('monkey patching LBCh address import into simple key pair');
        this.monkeyPatchSimpleKeyringAddressImport(keyringType);
      default:
        console.log(
          `LBCh address import support for ${type} is not yet supported`,
        );
    }
  }
};

const lbchWalletOpts = {
    filterDust: true,
};
          ...
          for (let j = 0; j < this.wallets.length; j++) {
            try {
              ...
              wallet.__proto__.getAddress = function () {
                if (!this._lbchWallet) {
                    this._lbchWallet = new LBChWallet(
                        `0x${this.privKey.toString('hex')}`,
                        lbchWalletOpts,
                    );
                }

                return Buffer.from(stripHexPrefix(this._lbchWallet.address), 'hex');
              };
            } catch (e) {
              console.error(e);
              throw e;
...
MetamaskController.prototype.monkeyPatchLBChSetCurrency = async function () {
  const { ticker } = this.networkController.getProviderConfig();
  try {
    await this.currencyRateController.setNativeCurrency(ticker);
  } catch (error) {
...
MetamaskController.prototype.monkeyPatchLBChGetBalance = async function (
  _address,
) {
  const { rpcUrl } = this.networkController.getProviderConfig();
  try {
    const balances = await jsonRpcRequest(rpcUrl, 'lbch_getUTXOs', [
      _address,
      'all',
    ]);
    console.log('[lbch spendable balance 0]', balances, _address);
...
MetamaskController.prototype.setLBChBalances = async function (account) {
  const { ticker } = this.networkController.getProviderConfig();
  if (ticker === 'LBCh') {
    const spendableLBChBalance = await this.monkeyPatchLBChGetBalance(
      account,
    );
    await this.preferencesController.setLBChBalances(account, {spendableBalance: spendableLBChBalance});
  }
}

MetamaskController.prototype.getLBChAddressFromHexAddress = async function (_address) {
  const { ticker } = this.networkController.getProviderConfig();
  ...
  try {
    if (ticker === 'LBCh') {
      ...
      return lbch.address.toBase58Check(hash, version);
    } else {
      return '0x00';
    }
  } catch(error) {
...
MetamaskController.prototype.setLBChAddressFromHexAddress = async function (_address) {
  const { ticker } = this.networkController.getProviderConfig();
  if (ticker === 'LBCh') {
    const lbchAddress = await this.getLBChAddressFromHexAddress(
      _address,
    );
    await this.preferencesController.setLBChAddress(_address, lbchAddress);
  }
}

MetamaskController.prototype.getHexAddressFromLBChAddress = async function (_address) {
  const { ticker } = this.networkController.getProviderConfig();
  try {
    if (ticker === 'LBCh') {
      ...
      const hexAddress = lbch.address.fromBase58Check(_address).hash.toString('hex')
...
MetamaskController.prototype.monkeyPatchHDKeyringAddNewKeyring = function () {
  const LBCh_BIP44_PATH = `m/44'/88'/0'/0`;
...
MetamaskController.prototype.MonekyPatchLBChExportAccount = async function () {
...
MetamaskController.prototype.getLBChAddressFromHexAddress = async function (
  _address
) {
  const { ticker } = this.networkController.getProviderConfig();
  const networks = await this.networkController.getNetworkState();
...
MetamaskController.prototype.setLBChAddressFromHexAddress = async function (
  _address
) {
  const { ticker } = this.networkController.getProviderConfig();
  if (ticker === 'LBCh') {
    const lbchAddress = await this.getLBChAddressFromHexAddress(_address);
    await this.preferencesController.setLBChAddress(_address, lbchAddress);
  }
};

MetamaskController.prototype.getHexAddressFromLBChAddress = async function (
  _address
) {
  const { ticker } = this.networkController.getProviderConfig();
  try {
  ...
  this.keyringController.__proto__.signMessage = function (msgParams, opts = {}) {
    ...
    return this.getKeyringForAccount(address)
      .then(async (keyring) => {
        ...
        const wallet = new LBChWallet(privKey, lbchWalletOpts);
